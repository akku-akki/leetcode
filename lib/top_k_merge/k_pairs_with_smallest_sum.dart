...
class Solution {
  List<List<int>> kSmallestPairs(List<int> nums1, List<int> nums2, int k) {
    ...
    HeapPriorityQueue<SmallestPairs> pQ =
        HeapPriorityQueue((p0, p1) => p1.sum - p0.sum);
    SmallestPairs _currentPair = SmallestPairs(0, 0, 0);
    for (int i = 0; i < nums1.length && i < k; i++) {
      for (int j = 0; j < nums2.length && j < k; j++) {
        final currentPair =
            SmallestPairs(nums1[i], nums2[j], nums1[i] + nums2[j]);
        _currentPair = currentPair;
        print(_currentPair);
        if (pQ.length < k) {
          pQ.add(currentPair);
        } else {
          final first = pQ.first;
          if (currentPair.sum > first.sum) {
            break;
          } else {
            pQ.removeFirst();
            pQ.add(currentPair);
    ...
    while (pQ.isNotEmpty) {
      ...
      result.add([current.firstElement, current.secondElement]);
    }
    return result;
  }
}
...
class SmallestPairs {
  ...
  SmallestPairs(this.firstElement, this.secondElement, this.sum);
  @override
  String toString() {
    return '[$firstElement,$secondElement,$sum]';
  }
}
