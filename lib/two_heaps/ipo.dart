...
class Solution {
  int findMaximizedCapital(int k, int w, List<int> profits, List<int> capital) {
    HeapPriorityQueue<List<int>> minCapital =
        HeapPriorityQueue((p0, p1) => p0[0] - p1[0]);

    HeapPriorityQueue<List<int>> maxProfit =
        HeapPriorityQueue((p0, p1) => p1[1] - p0[1]);

    for (int i = 0; i < profits.length; i++) {
      minCapital.add([capital[i], profits[i]]);
    }
    for (int i = 0; i < k; i++) {
      while (minCapital.isNotEmpty && minCapital.first[0] <= w) {
        maxProfit.add(minCapital.removeFirst());
      }
      if (maxProfit.isEmpty) {
        break;
      }
      w += maxProfit.removeFirst()[1];
    }
    return w;
  }
}
