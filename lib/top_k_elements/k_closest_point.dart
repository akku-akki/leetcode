...
class Solution {
  List<List<int>> kClosest(List<List<int>> points, int k) {
    final maxHeap = HeapPriorityQueue<Point>(
      (p0, p1) => getDistance(p1).compareTo(getDistance(p0)),
    );
    final List<List<int>> ans = [];
    for (final point in points) {
      maxHeap.add(Point(point[0], point[1]));
      if (maxHeap.length > k) {
        maxHeap.removeFirst();
      }
    }
    while (maxHeap.isNotEmpty) {
      final resultPoint = maxHeap.removeFirst();
      ans.add([resultPoint.x, resultPoint.y]);
    }
    return ans;
  }

  double getDistance(Point point1) {
    final xDiff = point1.x;
    final yDiff = point1.y;
    final distance = Math.sqrt(xDiff * xDiff + yDiff * yDiff);
