...
class Solution {
  String frequencySort(String s) {
    Map<String, StringFrequency> frequecncyCount = {};
    HeapPriorityQueue<StringFrequency> pQ =
        HeapPriorityQueue((p0, p1) => p1.frequency - p0.frequency);
    for (int i = 0; i < s.length; i++) {
      if (frequecncyCount.containsKey(s[i])) {
        frequecncyCount[s[i]]!.frequency++;
      } else {
        frequecncyCount[s[i]] = StringFrequency(s[i], 1);
      }
    }
    frequecncyCount.forEach((key, value) {
      pQ.add(frequecncyCount[key]!);
    });
    StringBuffer buffer = StringBuffer('');
    while (pQ.isNotEmpty) {
      StringFrequency current = pQ.removeFirst();
...
class StringFrequency {
  String char;
  int frequency;
  StringFrequency(this.char, this.frequency);
