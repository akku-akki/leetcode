...
class Solution {
  int minCostConnectPoints(List<List<int>> points) {
    ...
    List<int> cost = List.generate(n, (index) => 9999999999999999);
    ...
    for (int i = 0; i < n; i++) {
      int currentCost = 9999999999999999;
      int curPoint = -1;
      for (int j = 0; j < n; j++) {
        if (!visited[j] && cost[j] < currentCost) {
          currentCost = cost[j];
          curPoint = j;
        }
      }
      ...
      for (int i = 0; i < n; i++) {
        currentCost = getCost(points[i], points[curPoint]);
  ...
  int getCost(List<int> point1, List<int> point2) =>
